...
export default async function MobileHeritagePage(props: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="-mx-4 -mb-24 min-h-dvh bg-ink-900 px-4 pb-28 text-white">
      <header className="flex items-center gap-2 pb-3 pt-[calc(env(safe-area-inset-top)+0.5rem)]">
        <BackLink fallbackHref={`/m/place/${id}`} tone="glass" label={`Back to ${destination.name}`} />
