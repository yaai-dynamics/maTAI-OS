...
export default async function DestinationPage(props: { params: Promise<{ id: string }> }) {
  ...
  const experiences = getExperiencesFor(id);
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="-mx-4 -mt-5 sm:mx-0 sm:mt-0">
        <div className="relative overflow-hidden sm:rounded-2xl">
          ...
          <div className="absolute inset-x-0 bottom-0 p-5 sm:p-7">
            ...
            <h1 className="mt-2 text-[30px] font-semibold leading-tight tracking-tight text-white sm:text-[36px]">
              {destination.name}
            </h1>
          ...
          {heritage ? (
            <Card className="overflow-hidden">
              <div className="immersive flex flex-wrap items-center justify-between gap-4 px-5 py-5">
                ...
                <Link
                  href={`/explore/destinations/${id}/heritage`}
              ...
              <CardBody>
                <ul className="space-y-2">
                  {nearby.map((entry) => (
                    <li key={entry.id}>
                      <Link
                        href={`/explore/destinations/${entry.id}`}
