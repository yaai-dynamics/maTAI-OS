...
/** A centred overlay for detail that belongs on top of the current page, not instead of it. */
export function Modal({
  ...
  subtitle?: ReactNode;
  children: ReactNode;
  /** md ~ 32rem, lg ~ 42rem, xl ~ 64rem (room for a few columns), full ~ 72rem and nearly the viewport's height. */
  size?: 'md' | 'lg' | 'xl' | 'full';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      <div
        ...
        tabIndex={-1}
        className={cn(
          'relative flex w-full flex-col overflow-hidden rounded-2xl bg-paper shadow-overlay',
          size === 'full' ? 'max-h-[94vh]' : 'max-h-[88vh]',
          size === 'full'
            ? 'sm:max-w-6xl'
            : size === 'xl'
              ? 'sm:max-w-5xl'
      ...
      >
        <div className="flex shrink-0 items-start justify-between gap-4 border-b border-line px-5 py-3.5">
          ...
          <button
            type="button"
            onClick={onClose}
            className="shrink-0 rounded-md px-2 py-1 text-sm text-ink-600 hover:bg-surface-2 hover:text-ink-900"
          >
            Close
          </button>
        </div>
        <div className={cn('flex-1 overflow-y-auto', noBodyPadding ? '' : 'px-5 py-4')}>{children}</div>
      </div>
    </div>
  );
}
